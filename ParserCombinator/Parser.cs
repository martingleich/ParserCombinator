...
public readonly record struct ParseResult<T>
{
    ...
    public static ParseResult<T> Failed(ImmutableArray<Error> error, bool consumedAnything) => new(default!, consumedAnything, default, error);
    public static implicit operator ParseResult<T>(ParseResult.ParseResultFailed failed) => Failed(failed.Error, failed.ConsumedAnything);
    public TR Switch<TR>(Func<T, StringSlice, TR> good, Func<ImmutableArray<Error>, bool, TR> bad) => _errors.Length > 0 ? bad(_errors, _consumedAnything) : good(_value, _remaining);
    public ParseResult<TR> Select<TR>(Func<T, TR> good) => Switch((v,r) => ParseResult<TR>.Good(good(v), r), ParseResult<TR>.Failed);
    public ParseResult<TR> SelectGood<TR>(Func<T, StringSlice, ParseResult<TR>> good) => Switch(good, ParseResult<TR>.Failed);
    public ParseResult<T> SelectFailed(Func<ImmutableArray<Error>, bool, ParseResult<T>> failed) => Switch(Good, failed);
    public ParseResult<T> MarkConsumed() => Switch(ParseResult.Good, (e, _) => ParseResult.Failed(e, true));
    public override string ToString() => Switch((v, r) => $"{v} : '{r}'", (e, c) => $"Failed: {string.Concat(e)} {(c ? "tried" : "")}");
}

public static class ParseResult
{
    public readonly record struct ParseResultFailed(ImmutableArray<Error> Error, bool ConsumedAnything);
    public static ParseResultFailed Failed(ImmutableArray<Error> error, bool consumedAnything) => new(error, consumedAnything);
...
public static class Parser
{
    ...
    public static Parser<char> Satisfy(Func<char, bool> predicate, [CallerArgumentExpression(nameof(predicate))] string description = "") =>
    ...
    public static Parser<TR> SelectMany<T, TI, TR>(this Parser<T> parser, Func<T, Parser<TI>> selector, Func<T, TI, TR> resultSelector) => str =>
    ...
    public static Parser<T> Where<T>(this Parser<T> parser, Func<T, bool> predicate,
        [CallerArgumentExpression(nameof(predicate))] string description = "") => str =>
    ...
    public static Parser<TR> Sequence<T1, T2, TR>(Parser<T1> parser1, Parser<T2> parser2,
        Func<T1, T2, TR> resultSelector) => from p1 in parser1 from p2 in parser2
    ...
    public static Parser<TR> Sequence<T1, T2, T3, TR>(Parser<T1> parser1, Parser<T2> parser2, Parser<T3> parser3,
        Func<T1, T2, T3, TR> resultSelector) => from p1 in parser1 from p2 in parser2 from p3 in parser3
    ...
    public static Parser<T2> Surround<T1, T2, T3>(Parser<T1> parser1, Parser<T2> parser2, Parser<T3> parser3)
        => from p1 in parser1 from p2 in parser2 from p3 in parser3
        select p2;
    ...
    public static Parser<T> Skip<T, TS>(this Parser<T> parser, Parser<TS> skip) => from a in parser from _ in skip select a;
}